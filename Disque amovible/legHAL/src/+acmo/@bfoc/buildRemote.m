...
if ( PRF == 0 ) % automatic greatest PRF
    ...
    PRF = 1e6 / Nb_Tx_Per_Line ...
        / ((EventRxDur + system.hardware.MinNoop) * NbFocLines);
    obj = obj.setParam('PRF', PRF);
else % custom PRF
    PauseEnd = round((1e6 / PRF - EventRxDur*NbFocLines*Nb_Tx_Per_Line)) ;
end
...
TxCenter     = XOrigin + (FirstLinePos -1 + (0:(NbFocLines-1))*TxLinePitch)*system.probe.Pitch ;
