...
EventRxDur = EventRxDur * length(xApex);

% automatic greatest PRF
if PRF == 0
    Pause = system.hardware.MinNoop;
    PRF = 1e6 / (1 + PulseInv) ...
        / (EventRxDur + system.hardware.MinNoop * length(xApex));
...
% custom PRF
    ...
    Pause = round((1e6 / PRF - EventRxDur) / length(xApex)) ...
        / (1 + PulseInv);
end
...
elseif Pause > system.hardware.MaxNoop
    PRF = 1e6 / (1 + PulseInv) ...
        / (EventRxDur + system.hardware.MaxNoop * length(xApex));
    WarnMsg = ['PRF was changed from ' num2str(obj.getParam('PRF')) ...
        ' Hz to ' num2str(PRF) ' Hz.'];
    obj.WarningMessage( WarnMsg )
    obj = obj.setParam('PRF', PRF);
...
if FrameRateUF == 0
    ...
    if FrameRateUF > PRF
        FrameRateUF = PRF;
        WarnMsg = ['FrameRateUF must be <= PRF, so it was changed from ' num2str(obj.getParam('FrameRateUF')) ...
            ' Hz to ' num2str(FrameRateUF) ' Hz.'];
        obj.WarningMessage( WarnMsg )
        obj = obj.setParam('FrameRateUF', FrameRateUF);
    end
    PauseFr = Pause + 1e6/FrameRateUF - 1e6/PRF ;
    if PauseFr < system.hardware.MinNoop
        PauseFr = system.hardware.MinNoop;
        
        FrameRateUF = 1e6 / ( ( 1e6/PRF ) + PauseFr );
        WarnMsg = ['FrameRateUF was changed from ' num2str(obj.getParam('FrameRateUF')) ...
            ' Hz to ' num2str(FrameRateUF) ' Hz.'];
        obj.WarningMessage( WarnMsg )
        obj = obj.setParam('FrameRateUF', FrameRateUF);

    elseif PauseFr > system.hardware.MaxNoop
        PauseFr = system.hardware.MaxNoop;
...
% hack for acmo.acmo to not doing anything concerning the framerate
obj = obj.setParam('FrameRate', 0);

% ============================================================================ %

% Add the flat events
FLAT = elusev.circular( ...
    'TwFreq', TwFreq, ...
    'NbHcycle', NbHcycle, ...
    'Polarity', 1, ...
    'xApex', xApex, ...
