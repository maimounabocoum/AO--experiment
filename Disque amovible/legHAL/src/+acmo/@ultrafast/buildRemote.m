...
% Estimate Event duration (only based on RX duration)
if SyntAcq
    EventDurAllFlats = length(FlatAngles) * Nb_Tx_Per_Line * (EventRxDur+system.hardware.MinNoop) * 2;
else
    EventDurAllFlats = length(FlatAngles) * Nb_Tx_Per_Line * (EventRxDur+system.hardware.MinNoop);
end

% automatic greatest PRF
PauseDelta = 0; % Timing error due to integer Pause parameter
if PRF == 0
    Pause = system.hardware.MinNoop;
    PRF = 1e6 / ( EventDurAllFlats / length(FlatAngles) + Pause ) ;
...
% custom PRF
    ...
    Pause = round( 1e6 / PRF - EventDurAllFlats / length(FlatAngles) );
    PauseDelta = ( 1e6 / PRF - EventDurAllFlats / length(FlatAngles) ) - Pause; % Timing error due to integer Pause parameter
end
...
if FrameRateUF == 0
    ...
    FrameRateUF = PRF / length(FlatAngles);
    WarnMsg = ['Automatic FrameRateUF was set to ' num2str(FrameRateUF) ...
        ' Hz.'];
    obj.WarningMessage( WarnMsg )

else
    if FrameRateUF > PRF / length(FlatAngles)
        FrameRateUF = PRF / length(FlatAngles);
        WarnMsg = ['FrameRateUF must be <= PRF / length(FlatAngles), so it was changed from ' num2str(obj.getParam('FrameRateUF')) ...
            ' Hz to ' num2str(FrameRateUF) ' Hz.'];
        obj.WarningMessage( WarnMsg )
%         obj = obj.setParam('FrameRateUF', FrameRateUF);
    end
    PauseFrUF = Pause + 1e6/FrameRateUF - length(FlatAngles)*1e6/PRF ; % pause after each group of flat angles
    if PauseFrUF < system.hardware.MinNoop
        PauseFrUF = system.hardware.MinNoop;
        
        FrameRateUF = 1e6 / ( ( length(FlatAngles)*1e6/PRF ) + PauseFrUF );
...
PauseFrUF = PauseFrUF + PauseDelta*length(FlatAngles);
