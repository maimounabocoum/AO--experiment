% ELUSEV.ARBITRARY (ELUSEV.ELUSEV)
%   Create an ELUSEV.ARBITRARY instance.
%
%   OBJ = ELUSEV.ARBITRARY() creates a generic ELUSEV.ARBITRARY instance.
%
%   OBJ = ELUSEV.ARBITRARY(NAME, DESC, DEBUG) creates an ELUSEV.ARBITRARY
...
%
%   OBJ = ELUSEV.ARBITRARY(NAME, DESC, PARNAME, PARVALUE, ..., DEBUG) creates an
...
%
%   Dedicated functions:
%     - INITIALIZE builds the remoteclass ELUSEV.ARBITRARY.
%     - BUILDREMOTE builds the associated remote structure.
%
%   Inherited functions:
%     - ISPARAM checks if a parameter (or several parameters) already belongs to
%       the remoteclass ELUSEV.ARBITRARY.
%     - ADDPARAM adds a parameter/object (explicit definition and cell array
%       definition) to the remoteclass ELUSEV.ARBITRARY.
%     - GETPARAM retrieves the value/object of a ELUSEV.ARBITRARY parameter.
%     - SETPARAM sets the value of a ELUSEV.ARBITRARY parameter.
%     - ISEMPTY checks if all ELUSEV.ARBITRARY parameters are correctly defined.
%
%   Note - This class is defined as a member of the legHAL interface. It cannot
%   be used without all methods of the remoteclass ELUSEV.ARBITRARY and its
...
classdef arbitrary < elusev.elusev
...
methods ( Access = 'protected' )
    ...
    varargout = initalize(obj, varargin) % build remoteclass ELUSEV.ARBITRARY
...
% Class contructor
methods ( Access = 'public' )
    function obj = arbitrary(varargin)
        
        % Label of the object
        if ( length(varargin) > 1 )
            if ( ~ischar(varargin{1}) || ~ischar(varargin{2}))
                varargin = {'ARBITRARY', ...
                            'default arbitrary elementary ultrasound events', ...
                            varargin{1:end}};
            else
