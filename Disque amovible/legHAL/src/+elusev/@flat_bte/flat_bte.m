% ELUSEV.FLAT (ELUSEV.ELUSEV)
%   Create an ELUSEV.FLAT instance.
%
%   OBJ = ELUSEV.FLAT() creates a generic ELUSEV.FLAT instance.
%
%   OBJ = ELUSEV.FLAT(DEBUG) creates a generic ELUSEV.FLAT instance using the
%   DEBUG value (1 is enabling the debug mode).
%
%   OBJ = ELUSEV.FLAT(NAME, DESC, DEBUG) creates an ELUSEV.FLAT instance with
...
classdef flat_bte < elusev.elusev
...
methods ( Access = 'protected' )
    ...
    varargout = initalize(obj, varargin) % build remoteclass ELUSEV.FLAT
...
% Class contructor
methods ( Access = 'public' )
    function obj = flat_bte(varargin)
        
        % Label of the object
        if ( length(varargin) > 1 )
            if ( ~ischar(varargin{1}) || ~ischar(varargin{2}))
                varargin = {'FLAT', ...
                            'default flat elementary ultrasound events', ...
                            varargin{1:end}};
            else
