...
NThreads = 4 ;
% System parameters
IPaddress = '192.168.1.161'; % IP address of Aixplorer
...
RxFreq       = 4 * TwFreq; % MHz
RxCenter     = 40;          % mm
RxWidth      = 25;          % mm - no synthetic acquisition
RxBandwidth  = 2;          % 1 = 200%, 2 = 100%
EnsLength    = 11;          % # of repeated block
AngleRepFreq = 0;      % Hz
Scale        = 10 ; %cm/s
PRF          = Scale*10/(0.25*common.constants.SoundSpeed/1000/TwFreq)
% Hz
FrameRate    = 0;        % Hz
...
for m = 1 : NbRun
    for n = 1 : NbAcq
        BFStruct{n + (m-1) * NbAcq} = processing.lutSynthetic(SEQ,buffer(n, m).mode+1,ImgInfo);
        BFStruct{n + (m-1) * NbAcq}.Info.DebugMode = 0 ;
        BFStruct{n + (m-1) * NbAcq} = processing.reconSynthFlat(BFStruct{n + (m-1) * NbAcq}, buffer(n, m),NThreads);
    end
end
...
IQ = single(zeros(size(BFStruct{1}.IQ, 1), size(BFStruct{1}.IQ, 2), length(BFStruct) * size(BFStruct{1}.IQ, 3)));
for k = 1 : length(BFStruct)
    
    IQ(:,:, (1:size(BFStruct{k}.IQ, 3)) + (k-1) * size(BFStruct{1}.IQ, 3)) = single(BFStruct{k}.IQ);
...
% Display variables
R0RefValue      = 1E8; % 692407872 - gel value
R0CompressiondB = 20;
