% REMOTE.TX_FLAT (REMOTE.TX)
%   Create a REMOTE.TX_FLAT instance.
%
%   OBJ = REMOTE.TX_FLAT() creates a generic REMOTE.TX_FLAT instance.
%
%   OBJ = REMOTE.TX_FLAT(NAME, DESC, DEBUG) creates a REMOTE.TX_FLAT instance
...
%
%   OBJ = REMOTE.TX_FLAT(NAME, DESC, PARNAME, PARVALUE, ..., DEBUG) creates
%   a REMOTE.TX_FLAT instance with parameters PARSNAME set to PARSVALUE.
%
...
%
%   Dedicated functions:
%     - INITIALIZE builds the remoteclass REMOTE.TX_FLAT.
%     - SETDELAYS builds the REMOTE.TX_FLAT transmit delays.
%
%   Inherited functions:
%     - BUILDREMOTE builds the associated remote structure.
%     - ISPARAM checks if a parameter (or several parameters) already belongs to
%       the remoteclass REMOTE.TX_FLAT.
%     - ADDPARAM adds a parameter/object (explicit definition and cell array
%       definition) to the remoteclass REMOTE.TX_FLAT.
%     - GETPARAM retrieves the value/object of a REMOTE.TX_FLAT parameter.
%     - SETPARAM sets the value of a REMOTE.TX_FLAT parameter.
...
%   be used without all methods of the remoteclass REMOTE.TX_FLAT and its
...
methods ( Access = 'protected' )
    ...
    varargout = initalize(obj, varargin) % build remoteclass REMOTE.TX_FLAT
    varargout = setDelays(obj, varargin) % build the REMOTE.TX_FLAT delays
