...
% Update the tx delays
obj = obj.setDelays();

% Control delays dimension
TmpDelays = obj.getParam('Delays');
if ( ~isvector(TmpDelays) )
    
    % Build the prompt of the help dialog box
    ErrMsg = ['The TX delays should correspond to a vector of delays ' ...
        'for each tx channels.'];
    error(ErrMsg);
    
elseif ( length(TmpDelays) < system.hardware.NbTxChan )
    
    Delays = zeros(1, system.hardware.NbTxChan);
    Delays(1:length(TmpDelays)) = TmpDelays;
    
elseif ( length(TmpDelays) > system.hardware.NbTxChan )
    
    Delays = TmpDelays(1:system.hardware.NbTxChan);
    
else
    
    Delays = TmpDelays;
    if ( system.probe.NbElemts < system.hardware.NbTxChan )
        Delays(system.probe.NbElemts + 1 : end) = 0;
    end
