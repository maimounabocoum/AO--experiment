% ACMO.ULTRAPHASED (ACMO.ACMO)
%   Create an ACMO.ULTRAPHASED instance.
%
%   OBJ = ACMO.ULTRAPHASED() creates a generic ACMO.ULTRAPHASED instance.
%
%   OBJ = ACMO.ULTRAPHASED(DEBUG) creates a generic ACMO.ULTRAPHASED instance using
%   the DEBUG value (1 is enabling the debug mode).
%
%   OBJ = ACMO.ULTRAPHASED(NAME, DESC, DEBUG) creates an ACMO.ULTRAPHASED instance
%   with its name and description values set to NAME and DESC (character
%   values).
%
%   OBJ = ACMO.ULTRAPHASED(NAME, DESC, PARNAME, PARVALUE, ..., DEBUG) creates an
%   ACMO.ULTRAPHASED instance with parameters PARSNAME set to PARSVALUE.
%
...
%     - FR (single) sets the mode frame rate.
%       0 = greatest possible, [1 10000] Hz
...
%       [0 960] - default = 900
%     - PHASEDTGC (int32) enables the phased TGC mode.
%       0 = classical TGC, 1 = phased TGC - default = 0
...
%
%   Dedicated functions:
%     - INITIALIZE builds the remoteclass ACMO.ULTRAPHASED.
%     - BUILDREMOTE builds the associated remote structure.
%
%   Inherited functions:
%     - ISPARAM checks if a parameter (or several parameters) already belongs to
%       the remoteclass ACMO.ULTRAPHASED.
%     - ADDPARAM adds a parameter/object (explicit definition and cell array
%       definition) to the remoteclass ACMO.ULTRAPHASED.
%     - GETPARAM retrieves the value/object of a ACMO.ULTRAPHASED parameter.
%     - SETPARAM sets the value of a ACMO.ULTRAPHASED parameter.
%     - ISEMPTY checks if all ACMO.ULTRAPHASED parameters are correctly defined.
%
%   Note - This class is defined as a member of the legHAL interface. It cannot
%   be used without all methods of the remoteclass ACMO.ULTRAPHASED and its
...
classdef ultraphased < acmo.acmo
...
methods ( Access = 'protected' )
    ...
    varargout = initalize(obj, varargin) % build the remoteclass ACMO.ULTRAPHASED
...
%% Class contructor
methods ( Access = 'public' )
    function obj = ultraphased(varargin)
        
        % Label of the object
        if ( length(varargin) > 1 )
            if ( ~ischar(varargin{1}) || ~ischar(varargin{2}))
                varargin = {'ULTRAPHASED', ...
                            'default ultraphased acquisition mode', ...
                            varargin{1:end}};
            else
