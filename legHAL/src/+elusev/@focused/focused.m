% ELUSEV.FOCUSED (ELUSEV.ELUSEV)
%   Create an ELUSEV.FOCUSED instance.
%
%   OBJ = ELUSEV.FOCUSED() creates a generic ELUSEV.FOCUSED instance.
%
%   OBJ = ELUSEV.FOCUSED(DEBUG) creates a generic ELUSEV.FOCUSED instance using the
%   DEBUG value (1 is enabling the debug mode).
%
%   OBJ = ELUSEV.FOCUSED(NAME, DESC, DEBUG) creates an ELUSEV.FOCUSED instance with
%   its name and description values set to NAME and DESC (character values).
%
%   OBJ = ELUSEV.FOCUSED(NAME, DESC, PARNAME, PARVALUE, ..., DEBUG) creates an
...
%
%   Dedicated functions:
%     - INITIALIZE builds the remoteclass ELUSEV.FOCUSED.
%     - BUILDREMOTE builds the associated remote structure.
%
%   Inherited functions:
%     - ISPARAM checks if a parameter (or several parameters) already belongs to
%       the remoteclass ELUSEV.FOCUSED.
%     - ADDPARAM adds a parameter/object (explicit definition and cell array
%       definition) to the remoteclass ELUSEV.FOCUSED.
%     - GETPARAM retrieves the value/object of a ELUSEV.FOCUSED parameter.
%     - SETPARAM sets the value of a ELUSEV.FOCUSED parameter.
%     - ISEMPTY checks if all ELUSEV.FOCUSED parameters are correctly defined.
%
%   Note - This class is defined as a member of the legHAL interface. It cannot
%   be used without all methods of the remoteclass ELUSEV.FOCUSED and its
...
classdef focused < elusev.elusev
...
methods ( Access = 'protected' )
    ...
    varargout = initalize(obj, varargin) % build remoteclass ELUSEV.FOCUSED
...
% Class contructor
methods ( Access = 'public' )
    function obj = focused(varargin)
        
        % Label of the object
        if ( length(varargin) > 1 )
            if ( ~ischar(varargin{1}) || ~ischar(varargin{2}))
                varargin = {'FOCUSED', ...
                            'default focused elementary ultrasound events', ...
                            varargin{1:end}};
            else
