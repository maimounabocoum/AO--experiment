...
ImgInfo.FirstLinePos   = 65 ; % position of the first line in elt [>=1]
ImgInfo.txPitch        = 1 ; % txpitch [elements]
ImgInfo.nbTx           = 128 ; % txpitch [elements]
...
% Emission parameters
TwFreq          = 9/(1+ImgInfo.pulseInversion);      % transmit frequency [MHz]
...
%         buf = Sequence.getData('Realign', 1);
        buf = Sequence.getData('Realign', 2);
    else
