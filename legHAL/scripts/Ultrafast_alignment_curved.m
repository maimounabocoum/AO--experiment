...
ImagingVoltage = 30;             % imaging voltage [V]
...
UF.TxWidth      = piezoPitch*NActivPiezos;            % emission width [mm]
...
try
    ...
    while ~strcmpi(pressedKey, 'q');
        ...
        if length(absP) > 1
            ...
            p = polyfit(1:length(absP),absP,2); 
            ...
            figure (1)
            plot(y*1000);
            power10 = 4;%max(0,round(log10(max(abs(y*1000)))));
