function [dataCut,tsdata]=AcqGage(handle,AO)
...
dataCut = zeros(Prof,Nlignes);
dur = double(SEQ.InfoStruct.event(1).duration)*1e-6*acqInfo.SampleRate; % Duration of 1 event in # of points

for kk = 1:Nlignes
    
    dataCut(:,kk) = data(int32(dur*(kk-1)+1:dur*(kk-1)+Prof));
    dataCut(:,kk) = dataCut(:,kk) - mean(dataCut(1:100,kk));
end
