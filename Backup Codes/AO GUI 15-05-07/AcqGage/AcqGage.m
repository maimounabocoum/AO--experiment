...
if acqInfo.Mode == 1
    ...
    % Only 1 acquisition channel
elseif acqInfo.Mode == 2 % 2 acquisition channels
    for ii = 1:acqInfo.SegmentCount
        
        channel = 1:ChannelSkip:sysinfo.ChannelCount;
        
        % Get data
        transfer.Channel = channel(1); % Channel to be emptied
        ...
        % Get baseline
        transfer.Channel = channel(2); % Channel to be emptied
        
        transfer.Segment = ii; % number of the memory segment to be read
        [ret, baselinetmp, actual] = CsMl_Transfer(handle, transfer); % transfer
        % actual contains the actual length of the acquisition that may be
        % different from the requested one.
        CsMl_ErrorHandler(ret, 1, handle);
        
        
        data(:,MOD(ii,Nlignes),1)=data(:,MOD(ii,Nlignes),1)+(1/AO.actNTrig)*datatmp';
        data(:,MOD(ii,Nlignes),2)=data(:,MOD(ii,Nlignes),2)+(1/AO.actNTrig)*(baselinetmp)';
...
% 
% for ii=1:Nlignes
%     data(:,ii,1)=data(:,ii,1)-mean(data(1:100,ii,1));
%     data(:,ii,2)=data(:,ii,2)-mean(data(1:100,ii,2));
% end
