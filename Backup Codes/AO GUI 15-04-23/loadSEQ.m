...
if ~isempty( UnSetFields )
    for ii = 1: length( UnSetFields )
        if any(strcmpi(UnSetFields{ii},{'val','Nphase'})
            AO.( UnSetFields{ii} ) = 1;
...
if ~AO.hlOpen % Initialize buttons only if the window has been closed
    ...
    N_xText = uicontrol('Style','text',...
        'String','Period Step (Nb of elements)',...
        'visible','off',...
        'BackgroundColor',AO.parentcolor,...
        'horizontalalignment','center',...
        'Fontsize',12,...
    ...
    function Load(source,eventdata)
        ...
        switch get(ScanEdit,'value');
            ...
            case 3
                AO.N_x       = str2double(get(N_xEdit,'string'));
                AO.Nphase    = get(NphaseEdit,'value');
                
                AO.SEQ = AOSeqInit_STRUCT( AO );
        end
    ...
    function ScanSelect(source,eventdata)
        ...
        switch AO.val
            case 1
                ...
                set(N_xText,'visible','off')
                set(N_xEdit,'visible','off')
            ...
            case 3
                ...
                set(N_xText,'visible','on')
                set(N_xEdit,'visible','on')
    ...
    function browse(source,eventdata)
        switch AO.val
            ...
            case 3
                [filename,dir,FilterIndex] = uigetfile('D:\AcoustoOptique\SEQdir\SEQ_STR*.mat','Select a sequence');
        ...
        set(N_xEdit,'string',AO.N_x)
        set(NphaseEdit,'string',AO.Nphase)
    ...
    function checkValue(source,eventdata)
        ...
        if strcmp(get(source,'tag'),'N_x') || strcmp(get(source,'tag'),'Nphase')
            par = str2double(get(source,'string'))/2;
            np = get(NphaseEdit,'value')*2;
            
            if floor(par/np)-par/np<0
                display('Number of elements for spatial frequency steps must be a multiple of the number of phase')
                set(source,'string',num2str(round(par/np)*np))
            end
