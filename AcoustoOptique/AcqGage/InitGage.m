...
switch AO.val
    case 1
        % case focused waves
        Nlignes         = round(AO.ScanLength(AO.val)/system.probe.Pitch); % Number of lines of the scan
    case 2
        % case plane waves
        Nlignes         = length(AO.Param.FlatAngles); % Number of lines of the scan
        
    case 3
        % case plane waves
        Nlignes         = length(AO.Param.freqx)*length(AO.Param.FlatAngles)*AO.Nphase; % Number of lines of the scan
end
...
acqInfo.ExtClock        = 0;
acqInfo.Mode            = CsMl_Translate('Dual', 'Mode');%  Single, Dual, Quad or Octal
